...
const PRESET_SCENARIOS = [
  {
    ...
    tag: 'ALLOW',
    color: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/20 shadow-sm',
  ...
  },
  {
    ...
    tag: 'MODIFY',
    color: 'border-amber-500/30 bg-amber-500/10 text-amber-300 hover:bg-amber-500/20 shadow-sm',
  ...
  },
  {
    ...
    tag: 'BLOCK',
    color: 'border-red-500/30 bg-red-500/10 text-red-300 hover:bg-red-500/20 shadow-sm',
  ...
  },
  {
    ...
    tag: 'ESCALATE',
    color: 'border-purple-500/30 bg-purple-500/10 text-purple-300 hover:bg-purple-500/20 shadow-sm',
    ...
    >
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        ...
        <div className="xl:col-span-6 space-y-6 min-w-0">
          ...
          <div className="bg-[#0F172A]/90 border border-slate-800 rounded-2xl p-4 shadow-2xl">
            <div className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-2.5 flex items-center gap-1.5 font-mono">
            ...
            <div className="flex flex-wrap gap-2">
              {PRESET_SCENARIOS.map((sc, i) => (
                ...
                >
                  <span className="font-mono text-[9px] px-1.5 py-0.5 rounded-md bg-black/40 font-extrabold border border-current">
                    {sc.tag}
                  </span>
                  <span>{sc.label}</span>
                </button>
          ...
          <div className="bg-[#0F172A]/90 border border-slate-800 rounded-2xl p-5 shadow-2xl space-y-3.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-extrabold text-white uppercase tracking-wider flex items-center gap-1.5 font-mono">
                <Send className="w-3.5 h-3.5 text-blue-400" />
                Prompt Execution Interface
              </label>
              <span className="text-[10px] text-slate-400 font-mono hidden sm:inline-flex items-center gap-1">
                Press <kbd className="px-1.5 py-0.5 bg-slate-800 border border-slate-700 rounded-md text-slate-200 font-semibold">Ctrl + Enter</kbd> to run
              </span>
            </div>
            ...
            <textarea
              ...
              className="w-full text-xs font-mono bg-[#0B0F19] border border-slate-800 rounded-xl p-3.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none transition-all resize-y text-white leading-relaxed shadow-inner placeholder:text-slate-500"
            />

            <div className="flex items-center justify-between pt-1">
              <div className="text-xs text-slate-400 flex items-center gap-2 font-mono text-[11px]">
                <span className="text-slate-500">Endpoint:</span>
                <span className="font-semibold text-slate-200">{selectedVirtualModel?.name}</span>
          ...
          <div className="bg-[#0F172A]/90 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-4">
              ...
              {latestTrace && (
                <div className="flex items-center gap-3 text-xs font-mono">
                  ...
                  <button
                    ...
                    className="text-blue-400 hover:text-blue-300 font-bold flex items-center gap-1 hover:underline cursor-pointer bg-blue-500/10 px-2.5 py-1 rounded-lg border border-blue-500/20"
                  >
            ...
            {!latestTrace && !isExecuting && (
              <div className="py-16 text-center text-slate-400 text-xs flex flex-col items-center gap-2.5">
                ...
                <div className="font-semibold text-slate-200">No output generated yet</div>
            ...
            {isExecuting && !latestTrace && (
              <div className="py-12 text-center text-slate-300 text-xs space-y-2 animate-pulse">
                <div className="font-bold text-white text-sm font-mono">Evaluating Pipeline Governance...</div>
            ...
            {latestTrace && (
              <div className="space-y-4 animate-fade-in">
                ...
                {latestTrace.decision === 'ALLOW' && (
                  <div className="p-5 rounded-2xl border border-emerald-500/30 bg-emerald-950/20 space-y-3 shadow-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-[11px] font-mono text-slate-300 hover:text-white flex items-center gap-1 cursor-pointer bg-slate-900 px-2.5 py-1 rounded-md border border-slate-700"
                      >
                    ...
                    <div className="text-xs font-sans text-slate-100 bg-[#0B0F19] p-4 rounded-xl border border-slate-800 leading-relaxed shadow-inner whitespace-pre-wrap font-medium">
                ...
                {latestTrace.decision === 'MODIFY' && (
                  <div className="p-5 rounded-2xl border border-amber-500/30 bg-amber-950/20 space-y-3.5 shadow-lg">
                    <div className="flex items-center justify-between">
                      ...
                      {/* View Mode Tabs */}
                      <div className="flex bg-slate-900 p-0.5 rounded-xl text-[10px] font-semibold border border-slate-800">
                        <button
                    ...
                    {/* Diff Views */}
                    {modifyTab === 'redacted' && (
                      <div className="text-xs font-sans text-slate-100 bg-[#0B0F19] p-4 rounded-xl border border-slate-800 leading-relaxed shadow-inner whitespace-pre-wrap">
                        {latestTrace.finalResponse}
                      </div>
                    )}

                    {modifyTab === 'original' && (
                      <div className="text-xs font-mono text-red-200 bg-[#0B0F19] p-4 rounded-xl border border-red-900/60 leading-relaxed whitespace-pre-wrap">
                    ...
                    {modifyTab === 'split' && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                        <div className="p-3.5 bg-[#0B0F19] rounded-xl border border-red-900/60">
                          <div className="text-[10px] font-bold text-red-400 uppercase mb-1 font-mono">
                        ...
                        <div className="p-3.5 bg-[#0B0F19] rounded-xl border border-emerald-900/60">
                          <div className="text-[10px] font-bold text-emerald-400 uppercase mb-1 font-mono">
                            ControlPlane Sanitized (Delivered)
                          </div>
                          <div className="font-sans text-[11px] text-slate-100">
                            {latestTrace.finalResponse}
                          </div>
                        </div>
                      </div>
                ...
                {latestTrace.decision === 'BLOCK' && (
                  <div className="p-5 rounded-2xl border border-red-500/30 bg-red-950/20 space-y-3.5 shadow-lg">
                    ...
                    <div className="p-4 bg-[#0B0F19] rounded-xl border border-slate-800 text-xs font-mono text-slate-300 shadow-inner">
                      <div className="text-[10px] font-bold text-red-400 uppercase mb-1.5">
                ...
                {latestTrace.decision === 'ESCALATE' && (
                  <div className="p-5 rounded-2xl border border-purple-500/30 bg-purple-950/20 space-y-3.5 shadow-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <Link
                        href="/reviews"
                        className="text-xs font-bold text-purple-300 hover:text-white flex items-center gap-1 hover:underline bg-slate-900 px-3 py-1 rounded-lg border border-purple-500/30 shadow-md"
                      >
                    ...
                    <div className="p-3.5 bg-[#0B0F19] rounded-xl border border-slate-800 text-xs text-slate-300 shadow-inner">
                      <span className="font-bold text-white">Status:</span> This query has been placed in the compliance review hold queue with Risk Score <strong className="text-purple-400">{latestTrace.riskScore}/100</strong>. Response will not be dispatched until signed off by a designated risk officer.
                    </div>
                  </div>
                )}
              </div>
